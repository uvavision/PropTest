...
class AOKVQADataset(Dataset):
    IMAGE_PATH = {
        "train": ("train2017", "aokvqa_v1p0_train.json"),
        "test": ("val2017", "aokvqa_v1p0_val.json")}
    ...
    def __init__(self, split, data_path="",
                 ...
                 verbose=True, testing=False, max_samples=None, **kwargs):
        """
        ...
        self.img_data_path = '/scratch/COCO' #For tamara: '/scratch/COCO'  # For fuwen,tianlu : '/scratch/data/COCO'

        if verbose:
            path = self.data_path
            print(f"Start loading AOKVQA Dataset from {path}", flush=True)
        ...
        df['image_path'] = df["image_id"].apply(
            lambda x: f"{self.IMAGE_PATH[split][0]}/{x:012d}.jpg")
        ...
        if max_samples is not None:
            ...
            self.df = self.df.sample(n=max_samples, random_state=2024)
    ...
    def get_img_path(self, index):
        ...
        image_path = os.path.expanduser(os.path.join(self.img_data_path, image_id))
        return image_path
    ...
    def __getitem__(self, index):
        ...
        if not self.testing:
            answer_list = self.df.iloc[index]["direct_answers"]  # Return whole list
            selected_answers = self.df.iloc[index]["direct_answers"]  # Apply answer_selection() function to list of dict

        # Load and transform image
        image_path = os.path.expanduser(os.path.join(self.img_data_path, image_path))
        ...
        if self.testing:
            ...
            return {"sample_id": image_id, 'answer': selected_answers, "image": img, "query": question,
    ...
    def get_item_score(self, p, g):
        ...
        p = self.post_process(p, stem=False) # added stem=False

        # The formulation is explained here: https://visualqa.org/evaluation.html
        #num_match = sum([p == da for da in g])
        #item_score = min(1.0, num_match / 3.0)
        appear_count = g.count(p)
        acc = min(1.0, appear_count / 3)
        accs = []
    ...
    def get_sample_path(self, index):
        sample_name = self.df.iloc[index][f"{self.input_type}_path"]
        sample_path = self.data_path + sample_name
        return sample_path
