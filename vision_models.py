...
if "gpt" in config.codex.model or "gpt" in config.gpt3.model:
    client = OpenAI(api_key=CANDIDATE_API[CURRENT_API])
...
class GPTModel(BaseModel):
    ...
    def get_qa(self, prompts, prompt_base: str=None) -> list[str]:
        ...
        for p in prompts:
            question = p
            prompts_total.append(prompt_base.replace("INSERT_QUERY_HERE", question))
        ...
        if self.n_votes > 1:
            ...
            if self.model == 'chatgpt':
                ...
                response = self.process_answer(response)
        return response

    def get_qa_fn(self, prompt):
        response = self.query_gpt3(prompt, model=self.model, max_tokens=5, logprobs=1, stream=False,
                                   stop=["\n", "<|endoftext|>"], test_case=True)
        return response
    ...
    def query_gpt3(self, prompt, model="text-davinci-003", max_tokens=16, logprobs=None, stream=False,
                   stop=None, top_p=1, frequency_penalty=0, presence_penalty=0, test_case=False):
        ...
        if model == "chatgpt":
        ...
        elif model == "gpt-3.5-turbo-1106" or model =="gpt-3.5-turbo":
            if test_case:
                messages = [{"role": "system", "content": "Please only provide the answer in yes or no."},
            ...
            if test_case:
                responses = client.chat.completions.create(
                    ...
                    messages=[
                        {"role": "system",
                         "content": "Please only provide the answer in yes or no."},
                        {"role": "user", "content": prompt[0]}],
                    temperature=self.temperature,
                    max_tokens=max_tokens,
                    top_p=1.,
                ...
                print(f"Original Response: {responses.choices[0].message.content}")
    ...
    @classmethod
    def list_processes(cls):
        return ['gpt_' + n for n in ['qa', 'general', 'guess']]


@cache.cache(ignore=['result'])
def llama2_cache_aux(fn_name, prompts, result):
    """
    This is a trick to manually cache results from GPT-3. We want to do it manually because the queries to GPT-3 are
    batched, and caching doesn't make sense for batches. With this we can separate individual samples in the batch
    """
...
class Llama2Model(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        super().__init__(gpu_number=gpu_number)
        with open(config.llama2.qa_prompt) as f:
            self.qa_prompt = f.read().strip()

        with open(config.llama2.guess_prompt) as f:
            self.guess_prompt = f.read().strip()

        with open(config.llama2.general_prompt) as f:
            self.general_prompt = f.read().strip()

        if config.load_models.llama2_model in ("Llama-2-13b-chat", "Llama-2-34b-chat", "Llama-2-7b-chat"):
            model_id = f"meta-llama/{config.load_models.llama2_model}-hf"
        ...
        elif config.load_models.llama2_model in ("Llama-3-8B-Instruct"):
            model_id = f"meta-llama/Meta-{config.load_models.llama2_model}"
            self.tokenizer = AutoTokenizer.from_pretrained(model_id)
    ...
    def process_guesses_fn(self, prompt):
        # The code is the same as get_qa_fn, but we separate in case we want to modify it later
        response = self.guess_fwd_llama2(prompt)
        return response
    ...
    def get_qa(self, prompts, prompt_base: str = None) -> list[str]:
        ...
        response = self.process_answer(response).replace('a:','').replace(' ','')

        return response

    def get_qa_fn(self, prompt):
        response = self.query_llama2(prompt, test_case=True)
        return response

    def get_general(self, prompts) -> list[str]:
        prompt_base = self.general_prompt
        prompts_total = []

        for p in prompts:
            question = p
            prompts_total.append(prompt_base.replace("INSERT_QUERY_HERE", question))
        response = self.query_llama2(prompts_total)

        response = self.process_answer(response.replace('A:',''))
        return response

    def query_llama2(self, prompt, test_case=False):

        use_flask = False
        if test_case and use_flask:

            new_prompt = [[{"role": "system", "content": "Please only provide the answer in yes or no."},
                           {"role": "user", "content": prompt[0]}]]
            input_data = {"text": new_prompt}

            try:
                response = requests.post(self.server_url, json=input_data)

                if response.status_code == 200:
                    result = response.json()
                else:
                    print(f"Error calling model1: {response.status_code}")
                    result = None

                # Continue with your training code using input_data_from_model1
            ...
            if config.load_models.llama2_model in ("Llama-3-8B-Instruct"):
                input_data= [{"role": "system",
                           "content": "I am a highly intelligent question answering bot. My goal is to answer the way a typical person might, even if this is not entirely accurate, in very brief answers."},
                          {"role": "user", "content": prompt[0]},
                          ]
                try:
                ...
                input_data = prompt[0]
                try:
                    out = self.pipeline(input_data, do_sample=False, num_return_sequences=1,
                                eos_token_id=self.tokenizer.eos_token_id, max_length=256,)
                    if not test_case:
                        print("output of Llama2: \n", out[0]['generated_text'][len(prompt[0]):])
                    result = out[0]['generated_text'][len(prompt[0]):]
                    return result

                except Exception as e:
                    print(f"Error: {str(e)}")

    def guess_fwd_llama2(self, prompt, test_case=False):

        if config.load_models.llama2_model in ("Llama-3-8B-Instruct"):
            input_data = [{"role": "user", "content": prompt[0]},]
            ...
            try:
                out = self.pipeline(input_data, do_sample=False, num_return_sequences=1,
                                eos_token_id=self.tokenizer.eos_token_id, max_length=1280)
                print("output of Llama2: \n", out[0]['generated_text'][len(prompt[0]):])
                result = out[0]['generated_text'][len(prompt[0]):]
                return result
            except Exception as e:
    ...
    def forward(self, prompt, process_name):
        ...
        if len(prompt) > 0:
            if process_name == 'llama2_qa':
                response = self.get_qa(prompt)
            elif process_name == 'llama2_guess':
                response = self.process_guesses(prompt)
            else:  # 'gpt3_general', general prompt, has to be given all of it
                response = self.get_general(prompt)
        else:
        ...
        if config.use_cache:
            ...
            for p, r in zip(prompt, response):
                # "call" forces the overwrite of the cache
                llama2_cache_aux.call(process_name, p, r)
            for i, idx in enumerate(to_compute):
                results[idx] = response[i]
            results = results[0]
        else:
    ...
    @classmethod
    def list_processes(cls):
        return ['llama2_' + n for n in ['qa', 'general', 'guess']]


def run_code_llama(prompts, test_code=False):
    import requests

    # Define the URL of the Flask server
    server_url = "http://localhost:8000/generatecode" #8000, 8002, 8004, 8006
    # Preprocess prompts into a right format
    input_data_for_model2 = {"text": prompts}

    try:
        # Make a POST request to the Flask server to get input data from model1
        response = requests.post(server_url, json=input_data_for_model2)

        if response.status_code == 200:
            input_data_from_model1 = response.json()
        else:
            print(f"Error calling model1: {response.status_code}")
            input_data_from_model1 = None

        # Continue with your training code using input_data_from_model1
        if input_data_from_model1 is not None:
            # See if the generated code starts with def and ends with return
            gen_code = input_data_from_model1[0]['generation']['content']
            lines = gen_code.split('\n')
            # Added
            if "while" in gen_code:
                gen_code = gen_code.replace("while", "if")
            if test_code == False and 'def execute_command' not in lines[0] and gen_code.count("execute_command") == 0:
                ...
                lines = gen_code.split('\n')
            if test_code == False:
                print(f"Originally gen_code/gen_feedback before post-processing: \n {gen_code} \n")
            # Post_processing
            # For test code
            if test_code == True and ('def' not in lines[0] or 'return' not in lines[-1] or gen_code.count("return") > 1):
                #print(f"gen_code: {gen_code}")
                ...
                # Find the index of the line that starts with 'return ' after the start_index
                end_index = next(
                    (i for i, line in enumerate(lines[start_index:]) if line.strip().startswith('return ')),
                    None) if start_index is not None else None
                if start_index is not None and end_index is not None:
                    # Get the lines from start_index to end_index inclusive
                    selected_lines = lines[start_index:start_index + end_index + 1]

                    # Join the selected lines with '\n' to get the final string
                    final_string = '\n'.join(selected_lines)
                    input_data_from_model1[0]['generation']['content'] = final_string
                else:
                    print('The required lines were not found in the input string.')
                    print(f'{gen_code}')
            elif config.codex.eval_result == True and test_code == False and 'def' not in gen_code: # When the self-eval with results does not give new codes
                pattern = r"Python Code:\n(.*?)\nAssertion Code:"
                match = re.search(pattern, prompts[0][1]['content'], re.DOTALL)
                if match:
                    input_data_from_model1[0]['generation']['content'] = match.group(1).strip()
                    print(f'The code is already correct, so the original code is : \n{match.group(1).strip()}')
                else:
                    input_data_from_model1[0]['generation']['content'] = "correct"
            elif test_code == False and ('def' not in lines[0] or 'return' not in lines[-1] or gen_code.count("return") > 1): # For non-test code
                if "function:" in gen_code or "code:" in gen_code or "solution:" in gen_code:
                    for i, line in enumerate(lines):
                        if "function:" or "code:" or "solution:" in line:
                            gen_code = '\n'.join(lines[i+1:])
                            lines = gen_code.split('\n')
                            break
                ...
                for i in range(start_index, len(lines) - 1, 1):  # -1 to avoid index out of range on next line check
                    if ("return" in lines[i] and len(lines[i + 1]) > 1 and lines[i + 1][1] != ' ' and end_index is None) or lines[i + 1] ==  '```':  # when next line is not empty
                        end_index = i
                        break
                    elif "return" in lines[i] and lines[i + 1] == '' and end_index is None:
                        end_index = i
                        break
                    elif "return" in lines[i+1] and i == len(lines) - 2 and end_index is None:
                        end_index = i + 1
                        break
                if start_index is not None and end_index is not None:
                    # Get the lines from start_index to end_index inclusive
            ...
            return input_data_from_model1[0]['generation']['content']

    except Exception as e:
        print(f"Error: {str(e)}")
        return "["

def post_process_GPT(result):
    lines = result.split('\n')
    if '```' in lines[0] and '```' in lines[-1]:
        modified_string = '\n'.join(lines[1:-1])
    elif 'def' not in lines[0]:
        modified_string = 'def execute_command(image)->str:\n    '+ '\n'.join(lines)
    else:
...
# Define your token per minute limit
token_per_minute_limit = 300000
# Calculate the delay in seconds based on your limit
delay_seconds = (max_tokens_per_request / token_per_minute_limit) * 60


# @cache.cache
@backoff.on_exception(backoff.expo, Exception, max_tries=10)
def codex_helper(extended_prompt, test_case=False):
    global count, api_num, CURRENT_API, CANDIDATE_API
    assert 0 <= config.codex.temperature <= 1
    assert 1 <= config.codex.best_of <= 20

    if config.codex.model in ("gpt-4", "gpt-3.5-turbo", "gpt-3.5-turbo-1106", "gpt-4-0125-preview"):
        # Wait for the calculated delay before the next query
        time.sleep(delay_seconds)
        if not isinstance(extended_prompt, list):
            extended_prompt = [extended_prompt]
        if test_case:  # test case code generation
            responses = [openai.ChatCompletion.create(
                    model=config.codex.model,
                    messages=[
                        {"role": "system", "content": "You are an expert programming assistant. Only answer with a function starting with def execute_test."},
            ...
            resp = [r['choices'][0]['message']['content'].replace("solve_query(image)", "execute_command(image, my_fig, time_wait_between_lines, syntax)")
                            for r in responses]

        else:  # Code Generation (not test case)
            responses = [openai.ChatCompletion.create(
                    model=config.codex.model,
                    messages=[
            ...
            for r in responses:
                ...
                print(f"Num of Tokens: {r['usage']} ")

            resp = [post_process_GPT(r['choices'][0]['message']['content']).replace("execute_command(image)->str",
                                                                                    "execute_command(image, my_fig, time_wait_between_lines, syntax)") for r in responses]

    elif config.codex.model in ("gpt-4o"):
        # Wait for the calculated delay before the next query
        time.sleep(delay_seconds)
        if not isinstance(extended_prompt, list):
            extended_prompt = [extended_prompt]
        if test_case:  # test case code generation
            responses = [client.chat.completions.create(
                model=config.codex.model,
                messages=[
                    {"role": "system", "content": "User will give you a task with some instruction. Your job is follow the instructions as faithfully as you can. While answering think step-by-step and only answer with a function starting with def execute_test."},
                    {"role": "user", "content": prompt}],
                temperature=config.codex.temperature,
                max_tokens=config.codex.max_tokens,
                top_p=1.,
            ...
            resp = [[r.choices[0].message.content.replace("solve_query(image)",
            "execute_command(image, my_fig, time_wait_between_lines, syntax)").replace("```python\n", "").replace("\n```","")]
                    for r in responses]
            ...
            responses = [client.chat.completions.create(
                ...
                messages=[
                    {"role": "system",
                    "content": "User will give you a task with some instruction. Your job is follow the instructions as faithfully as you can. While answering think step-by-step and only answer with a function starting with def execute_command."},
                    {"role": "user", "content": prompt}],
                temperature=config.codex.temperature,
                max_tokens=config.codex.max_tokens,
                top_p=1.,
            ...
            resp = [[post_process_GPT(r.choices[0].message.content).replace("execute_command(image) -> str",
                                        "execute_command(image, my_fig, time_wait_between_lines, syntax)").replace("execute_command(image)->str",
                                            "execute_command(image, my_fig, time_wait_between_lines, syntax)")]
                    for r in responses]

    elif config.codex.model in ("CodeLlama-7b-Instruct"):
        if test_case:
            responses = [run_code_llama(
                [[{"role": "system", "content": "You are an expert programming assistant. Only answer with a function starting with def execute_test."}, # Use base Python (comparison, sorting) for basic logical operations, left/right/up/down, math, etc.
                  {"role": "user", "content": prompt}]], test_code=True) for prompt in extended_prompt]
            resp = [
                r.replace("solve_query(image)", "execute_command(image, my_fig, time_wait_between_lines, syntax)")
                for r in responses]
        else:
            responses = [run_code_llama(
                [[{"role": "system", "content": "You are an expert programming assistant. Only answer with a function starting def execute_command."}, # Added you are an expert. You are an expert programming assistant. Only answer with a function starting def execute_command.
                  {"role": "user", "content": prompt}]], test_code=False) for prompt in extended_prompt]
            if responses != ['Correct code'] or responses is not None:
                resp = [r.replace("execute_command(image)", "execute_command(image, my_fig, time_wait_between_lines, syntax)")
                        for r in responses]
            #resp = ['[' for p in extended_prompt]
...
class CodexModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        if config.codex.model in ("CodeLlama-13b-Instruct", "CodeLlama-34b-Instruct", "CodeLlama-7b-Instruct"):
            ...
            model_id = f"meta-llama/{config.codex.model}-hf"
        ...
        elif config.codex.model in ("CodeLlama-70b-Instruct"):
            model_id = f"meta-llama/{config.codex.model}-hf"
            self.tokenizer = AutoTokenizer.from_pretrained(model_id)
            self.model = AutoModelForCausalLM.from_pretrained(
                model_id,
        ...
        elif config.codex.model in ("Llama-3-8B-Instruct", "Llama-3-70B-Instruct"):
            model_id = f"meta-llama/Meta-{config.codex.model}"
            self.tokenizer = AutoTokenizer.from_pretrained(model_id)
            self.model = AutoModelForCausalLM.from_pretrained(
                model_id,
                torch_dtype=torch.bfloat16,
    ...
    def post_process_codellama7B(self, gen_code, test_case=False):
        print(f"Originally gen_code: \n {gen_code}")
        gen_code = gen_code.replace("->  ImagePatch", "")
        # Finding the indices of the start of each "def" in the lines
        def_indices = [i for i, line in enumerate(gen_code.split('\n')) if line.strip().startswith('def')]

        # If there are at least two "def" lines, extract the lines from the first "def" to just before the second "def"
        if len(def_indices) > 1:
            lines = gen_code.split('\n')
            # Extract lines from first "def" to just before the second "def"
            extracted_lines = '\n'.join(lines[def_indices[0]:def_indices[1]])
            return extracted_lines
        else:
            # Return a message if there are less than two "def" lines
            return gen_code

    def post_process_codellama70B(self, gen_code, test_case=False):

        if test_case:
            # Postprocess for test code
            print(f"Originally gen_code: \n {gen_code}")
            gen_code = gen_code.replace("solve_query(image)",
                                        "execute_command(image, my_fig, time_wait_between_lines, syntax)")
            # Finding the indices of the start of each "def" in the lines
            lines = gen_code.split('\n')
            ...
            for line in lines:
                ...
                if stripped_line.startswith("def execute_test("):
                    inside_function = True

                if inside_function:
                    function_lines.append(line)
                    # Stop if we encounter [INST] after starting the function
                    if "return" in stripped_line and len(function_lines) > 1:
                        break
            if function_lines:
                adjusted_out = '\n'.join(function_lines)
                return adjusted_out
            else:
                print("No execute_test function found.")
                return None
        else:
            print(f"Originally gen_code: \n {gen_code}")
            gen_code = gen_code.replace("->", "").replace("str", "")
            # Finding the indices of the start of each "def" in the lines
            lines = gen_code.split('\n')
            inside_function = False
            function_lines = []
            ...
            for line in lines:
                stripped_line = line.strip()
                if stripped_line.startswith("def execute_command("):
                    inside_function = True

                if inside_function:
                    function_lines.append(line)
                    # Stop if we encounter [INST] after starting the function
                    if stripped_line.startswith("[INST]") and len(function_lines) > 1:
                        break

            if function_lines:
                # Remove the last [INST] line if it's added to function_lines
                if function_lines[-1].strip().startswith("[INST]"):
                    function_lines.pop()
                # Determine the base indentation of the function body
                base_indent = len(function_lines[1]) - len(function_lines[1].lstrip())
                # Adjust indentation
                adjusted_lines = [function_lines[0].strip()] + [line[base_indent-4:] if line.startswith(' ' * base_indent) else line for line in function_lines[1:]]
                adjusted_out = '\n'.join(adjusted_lines)
                adjusted_out = adjusted_out.replace("execute_command(image)",
                                            "execute_command(image, my_fig, time_wait_between_lines, syntax)")
                return adjusted_out
            else:
                return "No execute_command function found."

    def codeGen_llama(self, extended_prompt, test_case=False):
        assert 0 <= config.codex.temperature <= 1
        assert 1 <= config.codex.best_of <= 20
        ...
        for p in extended_prompt:
            try:
                prompt = f"<s>[INST] <<SYS>>\\n{system}\\n<</SYS>>\\n\\n{p}[/INST]"
                inputs = self.tokenizer(prompt, return_tensors="pt").to("cuda") #to("cuda") to("cuda:0")
                output = self.model.generate(inputs["input_ids"], max_new_tokens=256, do_sample=False, top_p=1) # temperature=0
                final_out = self.tokenizer.decode(output[0])

                match = re.search(pattern, final_out, re.DOTALL)  # final_out
                if match:
                    match_ = match.group(1).strip()
                    gen_code.append([self.post_process(match_, test_case)])
                else:
                    idxx = final_out.find('[/INST]')
                    gen_code.append([self.post_process(final_out[idxx + 8:], test_case)])
            except Exception as e:
                print(f"Error: {str(e)}")
                gen_code.append("[")
        return gen_code
    ...
    def codellama70b_codegen(self, extended_prompt, test_case=False):
        assert 0 <= config.codex.temperature <= 1
        assert 1 <= config.codex.best_of <= 20
        ...
        for p in extended_prompt:
            try:
                if config.codex.model in ("CodeLlama-70b-Instruct"):
                    ...
                    gen_code.append(self.post_process_codellama70B(results[0]['generation']['content']))
                else:
                    sequences = self.pipeline(p, do_sample=False,num_return_sequences=1,
                                               eos_token_id=self.tokenizer.eos_token_id,max_length=256,)
                    for seq in sequences:
                        gen_code.append([self.post_process(seq['generated_text'], test_case)])
            except Exception as e:
                print(f"Error: {str(e)}")
                gen_code.append("[")
        return gen_code
    ...
    def forward(self, prompt, input_type='image', prompt_file=None, base_prompt=None, test_case=False, assert_prompt=None, visual_info=None, image_path=None, original_code=None):
        ...
        if assert_prompt is not None and isinstance(prompt, list):
            extended_prompt = [base_prompt.replace("INSERT_ASSERTION_TESTS_HERE", assert_prompt[i]).
                           replace("INSERT_QUERY_HERE", prompt[i]) for i in range(len(prompt))]

        elif isinstance(prompt, list) and assert_prompt is None:
        ...
        result = self.forward_(extended_prompt, test_case)
    ...
    def forward_(self, extended_prompt, test_case):
        try:
            if config.codex.model in ("CodeLlama-13b-Instruct", "CodeLlama-34b-Instruct", "CodeLlama-7b-Instruct"):
                response = self.codeGen_llama(extended_prompt, test_case)
            elif config.codex.model in ('Llama-3-8B-Instruct', 'Llama-3-70B-Instruct'):
                response = self.llama3_codeGen(extended_prompt, test_case)
            elif config.codex.model in ("CodeLlama-70b-Instruct", "CodeLlama-70b-hf"):
                response = self.codellama70b_codegen(extended_prompt, test_case)
            else:
                response = codex_helper(extended_prompt, test_case)
...
class BLIPModel(BaseModel):
    ...
    def qa(self, image, question):
        ...
        generated_text = [self.process_answer(ans) for ans in generated_text] # BLIP - remove for aokvqa
        return generated_text
