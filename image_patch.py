...
def llm_query(query, context=None, long_answer=True, queues=None):
    ...
    """
    if long_answer:
        return forward(model_name='llama2_general', prompt=query, queues=queues)
    else:
        return forward(model_name='llama2_qa', prompt=[query, context], queues=queues)

def process_guesses(prompt, guess1=None, guess2=None, queues=None):
    return forward(model_name='llama2_guess', prompt=[prompt, guess1, guess2], queues=queues)
