...
## Added for In-context Retrivial
def get_InContext_Examples():
    # Load the database
    InContext_dataset = pd.read_csv("./CRAFT_Code_Database.csv")
    # Get the examples
    InContext_examples = InContext_dataset['example'].tolist()
    return InContext_examples

# Added for post-processing
def format_and_check_string(s):
    # Splitting the string into lines
    lines = s.split('\n')

    # Flag to check if formatting is needed
    needs_formatting = False

    # Processing each line
    formatted_lines = []
    for index, line in enumerate(lines):
        # Check if the line already has the correct spacing
        if index == 0 and not line.startswith(' '):  # First line should start with one space
            needs_formatting = True
        elif index > 0 and not line.startswith('    '):  # Other lines should start with four spaces
            needs_formatting = True

        # Strip leading and trailing whitespaces
        stripped_line = line.strip()

        # Add one space before the first character of the first line
        # and four spaces after each newline for subsequent lines
        if index == 0:  # For the first line
            formatted_line = ' ' + stripped_line
        else:
            formatted_line = '    ' + stripped_line if stripped_line else stripped_line
    ...
    return formatted_string, needs_formatting